...
void Stanley_Querregler_step(void)
{
  ...
  if (Stanley_Querregler_B.In1.Data) {
    ...
    if (rtb_ms == 1.0) {
      ...
      if (Stanley_Querregler_B.EnabledSubsystem_p.In1.Data >
        ...
        tmp = Stanley_Querregler_B.EnabledSubsystem_p.In1.Data;
      } else {
        tmp = Stanley_Querregler_P.Gain_Gain *
          Stanley_Querregler_B.EnabledSubsystem_p.In1.Data;
      }

      rtb_ms = -(atan(-(d_idx_0 * sin(Stanley_Querregler_B.refPose[2]) - d_idx_1
                        * cos(Stanley_Querregler_B.refPose[2])) *
                      Stanley_Querregler_P.LateralControllerStanley_Positi /
                      (tmp + 1.0)) + (Stanley_Querregler_B.b -
                  3.1415926535897931));
    } else {
      if (rtb_ms == 1.0) {
      ...
      rtb_ms = atan(-(d_idx_0 * sin(Stanley_Querregler_B.refPose[2]) - d_idx_1 *
                      cos(Stanley_Querregler_B.refPose[2])) * rtb_ms / (tmp +
        -1.0)) + (Stanley_Querregler_B.b - 3.1415926535897931);
    }

    rtb_ms *= 57.295779513082323;
    d_idx_0 = fabs(rtb_ms);
    if (rtb_ms < 0.0) {
      rtb_ms = -1.0;
    } else if (rtb_ms > 0.0) {
    ...
    if ((!(d_idx_0 < Stanley_Querregler_P.Kinematic_MaxSteeringAngle)) &&
        (!rtIsNaN(Stanley_Querregler_P.Kinematic_MaxSteeringAngle))) {
      d_idx_0 = Stanley_Querregler_P.Kinematic_MaxSteeringAngle;
    }
    ...
    if (rtb_ms > Stanley_Querregler_P.Sttigung_UpperSat) {
      rtb_ms = Stanley_Querregler_P.Sttigung_UpperSat;
    } else {
      if (rtb_ms < Stanley_Querregler_P.Sttigung_LowerSat) {
        rtb_ms = Stanley_Querregler_P.Sttigung_LowerSat;
      }
    }
    ...
    /* Gain: '<S7>/Bogenmaß' */
    Stanley_Querregler_B.Bogenma = Stanley_Querregler_P.Bogenma_Gain * rtb_ms;
  }
  ...
  if (Stanley_Querregler_B.In1.Data) {
     ...
     */
    rtb_BusAssignment.Data = static_cast<real32_T>(Stanley_Querregler_B.Bogenma);
  } else {
